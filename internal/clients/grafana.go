...
const (
	// Grafana Terraform Provider HCL configuration keys
	keyAuth  = "auth"
	keyOrgID = "org_id"
	keyURL   = "url"

	// Grafana Client environment variable keys
...
func TerraformSetupBuilder(version, providerSource, providerVersion string) terraform.SetupFn {
	return func(ctx context.Context, client client.Client, mg resource.Managed) (terraform.Setup, error) {
		...
		if v, ok := grafanaCreds[keyURL]; ok {
			ps.Configuration[keyURL] = v
		}

		// set provider configuration
		if v, ok := grafanaCreds[keyAuth]; ok {
			ps.Configuration[keyAuth] = v
		}
		...
		ps.Env = []string{
			fmt.Sprintf(fmtEnvVar, envAuth, grafanaCreds[keyAuth]),
			fmt.Sprintf(fmtEnvVar, envOrgID, orgid),
			fmt.Sprintf(fmtEnvVar, envURL, grafanaCreds[keyURL]),
		}
		return ps, nil
	}
}
